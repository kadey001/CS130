...
void application::draw_line_MPA(int x0, int y0, int x1, int y1, const vec3& linecolor)
{
    ...
    if(abs(y1 - y0) < abs(x1 - x0)) {
        if(x0 > x1) {
            ...
            draw_line_MPA_low(x0, y0, x1, y1, linecolor);
        }
    } else {
        if(y0 > y1) {
            draw_line_MPA_high(x1, y1, x0, y0, linecolor);
        } else {
...
void application::draw_line_MPA_high(int x0, int y0, int x1, int y1, const vec3& linecolor) {
    float dx = x1 - x0;
    float dy = y1 - y0;
    int xi = 1;
